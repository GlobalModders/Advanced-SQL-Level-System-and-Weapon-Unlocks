...
public plugin_init()
{
	...
	RegisterHam(Ham_Spawn, "player", "get_dualkatana", 1);
	...
	register_clcmd("say /resetskills", "remove_dualkatana")
}

public plugin_precache()
{
	new skillid[7];
	create_skill("Dual Katanas", MAX_LEVEL, skillid, REQ_LEVEL);
...
public get_dualkatana(id, Zombie)
{
	if(Zombie) return
	{
	g_Had_DualKatana[id] = 0
	...
	set_pev(id, pev_weaponmodel2, "")
			...
			if(!is_user_alive(id))
				return
			if(!get_skill_level(id, skill_id))
				return 
